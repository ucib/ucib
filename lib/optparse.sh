# Command-line option parsing functions for UCIB.
#
...
# specified option.
#
# Option parsing in UCIB is a little... arcane.  The problem is that we
...
parseopt() {
	...
	if [ "$hasvalue" != "true" ] && [ "$hasvalue" != "false" ]; then
	...
	for i in $(seq 0 $((${#ARGV[@]}-1))); do
		if [ "${ARGV[$i]}" = "--$optname" ]; then
			...
			debug "Remaining uparsed arguments: '${ARGV[*]}'"
